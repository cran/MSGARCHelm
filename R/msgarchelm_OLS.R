##### Function 'msgarchelm_OLS' ###################
requireNamespace("nnfor","MSGARCH", "forecast")
msgarchelm_OLS <- function(data, stepahead=10, nlags=5, modelcomb=c("sGARCH", "gjrGARCH"), distcomb=c("norm", "std"), freq = frequency(data), hn=10, est=c("lm"), rep=20, combt=c("mean")){
  ...
  ols_model <- lm(data_test ~ fcast_matrix)
  weights <- unname(ols_model$coef[-1])
  intercept <- unname(ols_model$coef[1])
  fitted_combined <- unname(fitted(ols_model))
